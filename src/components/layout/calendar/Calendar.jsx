...
function Calendar() {
  ...
  }, [isModalOpen, bookModalOpen, year, month]);
  ...
  const getSeasonType = (dateStr) => {
    return seasonList.find(season => dateStr >= season.startDate && dateStr <= season.endDate)?.seasonTypeName || null;
  };

  const dayCellClassNames = useCallback((arg) => {
    const dateStr = formatDateToYYYYMMDD(arg.date);
    const todayStr = formatDateToYYYYMMDD(new Date());
    const holidayDates = holidays.map(h => formatDateToYYYYMMDD(h.locdate.toString().replace(/(\d{4})(\d{2})(\d{2})/, "$1-$2-$3")));
    const classes = [];
    if (new Date(arg.date).getMonth() + 1 === month + 1) classes.push("next-month");
  ...
  const isPrevDisabled = useMemo(() => {
    ...
    return !isAdmin && (year < today.getFullYear() || (year === today.getFullYear() && month <= today.getMonth() + 1));
  }, [year, month, isAdmin]);
  ...
  const handleEventClick = ({ event }) => {
    ...
    if (!isAdmin && event.extendedProps.booking) return toast.error("이미 예약된 방입니다.");
    const isHoliday = holidays.some(h => formatDateToYYYYMMDD(`${h.locdate}`.replace(/(\d{4})(\d{2})(\d{2})/, "$1-$2-$3")) === dateStr);
    setRoomData({ ...room, startDate: dateStr, holiday: isHoliday });
    ...
    <>
      <div className="calendar-header bg-white p-4 rounded-t-lg flex justify-between items-center">
        ...
        <div className="toolbar flex gap-3">
          {isAdmin && (
            <button onClick={() => setIsModalOpen(true)} className="btn-settings p-3 bg-gray-200 hover:bg-gray-300 rounded-full text-gray-600 hover:text-gray-800">
              <BsGearFill className="text-xl" />
            </button>
          )}
          <div className="flex items-center gap-2">
            <button onClick={() => handleNav("prev")} disabled={isPrevDisabled} className="btn-prev p-3 rounded-full text-yellow-300 bg-yellow-700 hover:bg-yellow-800 disabled:opacity-50">
              <BsCaretLeftFill className="text-xl" />
            </button>
            <button onClick={() => handleNav("today")} className="btn-today px-6 py-2 text-base font-medium text-yellow-300 bg-yellow-700 hover:bg-yellow-800 rounded-lg">
              Today
            </button>
            <button onClick={() => handleNav("next")} className="btn-next p-3 rounded-full text-yellow-300 bg-yellow-700 hover:bg-yellow-800">
      ...
      <FullCalendar
        ...
        dayCellContent={({ date }) => {
          ...
          const badgeInfo = holidays.find(h => formatDateToYYYYMMDD(`${h.locdate}`.replace(/(\d{4})(\d{2})(\d{2})/, "$1-$2-$3")) === dateStr) || {};
          const badgeText = badgeInfo.dateName || getSeasonType(dateStr);
          const badgeColor = badgeInfo.dateName ? "bg-red-500" :
            badgeText === "성수기" ? "bg-orange-500" :
            badgeText === "준성수기" ? "bg-yellow-400 text-black" :
            badgeText === "비수기" ? "bg-blue-400" : "";

          return (
            <div className="flex justify-between px-1">
              <span   className={`text-xs text-white px-1.5 py-0.5 rounded ${badgeColor} truncate`}>{badgeText}</span>
              <span className="text-sm font-medium">{date.getDate()}</span>
            </div>
          );
        }}
        eventClassNames={({ event }) => event.extendedProps.booking ? ["reserved-room"] : []}
        eventMouseEnter={(info) => info.el.style.cursor = "pointer"}
        eventMouseLeave={(info) => info.el.style.cursor = "default"}
        eventClick={handleEventClick}
      />

      {isModalOpen && <SettingsModal onClose={() => setIsModalOpen(false)} onSave={() => setIsModalOpen(false)} />}
      {bookModalOpen && <BookingModal data={roomData} onClose={() => setBookModalOpen(false)} />}
    </>
  );
}
